	...
	for _, dialect := range []struct{ kind, env string }{{"sqlite", ""}, {"mysql", "TEST_MYSQL_DSN"}, {"postgres", "TEST_POSTGRES_DSN"}} {
		t.Run(dialect.kind, func(t *testing.T) {
			...
			cases := []struct {
				...
				// This fork disables the trust bypass: every wallet request
				// reserves atomically regardless of the configured threshold.
				{"above threshold still reserves", 10, 1, 5500000, 5500000, false, false, 1500},
				...
				{"fractional threshold still reserves", 10.5, 1, 5250001, 5250001, false, false, 1500},
				{"unlimited token still reserves", 10, 1, 5500000, 0, false, true, 1500},
