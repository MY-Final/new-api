...
import type { UserUsageDaily } from '../../types'
...
export function UserUsageChart(props: UserUsageChartProps) {
  ...
  const chartData = useMemo(
    () =>
      props.data.map((item) => ({
        day: item.day,
        value: props.metric === 'tokens' ? item.total_tokens : item.net_quota,
      })),
  ...
  return (
    <section className='overflow-hidden rounded-lg border'>
      ...
      <div className='h-52 p-1.5 sm:h-60 sm:p-2'>{chartContent}</div>
    </section>
  )
}
