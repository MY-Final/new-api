...
const billingSkeletonKeys = ['one', 'two', 'three', 'four', 'five']
      ...
      <div className='min-h-0 space-y-3'>
        ...
        <div className='max-h-[min(54vh,520px)] overflow-y-auto pr-1'>
          ...
          {!loading && records.length > 0 && (
            <div className='space-y-3'>
              {records.map((record) => {
                const isRedemption = record.record_type === 'redemption'
                const identifier = isRedemption
                  ? record.redemption_key || `#${record.redemption_id}`
                  : record.trade_no
                let statusConfig: {
                  label: string
                  ...
                  statusConfig = {
                    label:
                      record.status === 'refunded'
                        ...
                        : t('Redeemed'),
                    variant:
                      record.status === 'refunded' ? 'danger' : 'success',
                  }
                } else {
                  const status: TopupStatus =
                    record.status === 'used' ? 'pending' : record.status
                    ...
                    <div className='flex items-start justify-between gap-2'>
                      <div className='flex-1 space-y-1'>
                        ...
                        <div className='text-muted-foreground text-xs'>
                          {formatTimestamp(
                            record.redeemed_time || record.create_time
                          )}
                        </div>
                      </div>
                      <StatusBadge
