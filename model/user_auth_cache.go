...
if ARGV[10] == '1' then
  redis.call('HSET', KEYS[1],
    'Quota', ARGV[11], 'BonusQuota', ARGV[12], 'PaidQuota', ARGV[13])
end
...
return 1`
	result, err := common.RDB.Eval(context.Background(), script,
		...
		user.Username, user.Setting, user.CacheSchema, includeQuotaArg, user.Quota, user.BonusQuota, user.PaidQuota, ttl,
	).Int()
	if err != nil {
		return err
	}
